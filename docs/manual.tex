...
int main()
{
	...
	// In the example the W matrix is fully dense, so this
	// is a shortcut.
...
\begin{lstlisting}[caption={Specifying a custom problem in Python.},
                   ...
                   language={Python}]
import numpy as np
from lbdapy import DeterministicEquivalent, ProblemData
from lbdapy.cutfamilies import LooseBenders
...
print(master.second_stage_objective())
\end{lstlisting}
Once the problem is specified, the solution method is the same as for the other examples. Observe that arbitrary constraints senses may be specified.
