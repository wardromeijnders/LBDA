    ...
    double *beta, double gamma, double tau, double *x, double theta, double tol)
{
    ...
    for (size_t var = 0; var != d_n1; ++var)
    {
      d_model.addConstr(eta - d_xVars[var] - bigM * y_plus[var], GRB_LESS_EQUAL,
    -x[var]);       // eta <= x - x_bar + My d_model.addConstr(eta +
    d_xVars[var] - bigM * y_minus[var], GRB_LESS_EQUAL, x[var]);       // eta <=
    x_bar - x + My
    }
        // binary variables should sum to 2*d_n1 - 1, so that precisely one of
    the above constraints is binding,
        // since eta is chosen as large as possible by the solver, eta = ||x -
    x_bar|| double ones[d_n1]; fill_n(ones, d_n1, 1); GRBLinExpr y_sum;
    ...
    */
    ...
    int cind[2 * d_n1];
    iota(cind, cind + 2 * d_n1, numVars);
    double cval[2 * d_n1];
    fill_n(cval, 2 * d_n1, 1);
