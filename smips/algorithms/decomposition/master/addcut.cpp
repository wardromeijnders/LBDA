...
bool Master::addCut(double *beta, double gamma, double *x, double theta, double tol)
{
  ...
  if (betaxgamma > theta + tol) // then add cut and return false
  {
    ...
    GRBLinExpr lhs = GRBLinExpr(d_theta, -1.0); // expression with coefficient = -1.0 for theta
    lhs.addTerms(beta, d_xVars, d_n1); // expression is now of the form -theta + beta^T x
    d_model.addConstr(lhs, GRB_LESS_EQUAL, -gamma);
    */
    
        // adding the cut to d_cmodel
    GRBaddvar(d_cmodel, 0, NULL, NULL, 0, 0, 1e20, GRB_CONTINUOUS, NULL);  // slack
    
    size_t slackIdx = d_n1 + d_nSlacks; // slack variable index (there are d_n1 + 1 + nSlacks variables in the model)
    
    int cind[d_n1 + 2];
    iota(cind, cind + d_n1 + 1, 0);
    
     
    cind[d_n1 + 1] = slackIdx;  // refers to the latest variable (i.e. the slack)
    double cval[d_n1 + 1];
    for (size_t var = 0; var != d_n1; ++var)
      cval[var + 1] = -beta[var];
    cval[0] = 1;
