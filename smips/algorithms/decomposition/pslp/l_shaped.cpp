...
void Pslp::l_shaped(Master &master, size_t maxRounds, double tol)
{
  ...
  while (not stop)
  {
    ...
    for (size_t scenario = 0; scenario != d_S; ++scenario)
    {
      ...
      sub.solve(d_x, d_theta, master.d_cmodel, master.d_kappa, master.d_beta, master.d_gamma, maxRounds);  // generate cutting planes 
      sub.update(d_x, d_theta + zval);                           // change rhs (theta' = theta + z)
      sub.optimize();                                            // optimize (compute dual multipliers)
      sub.subgradient(gamma, kappa, nu, d_probs[scenario]);      // compute subgradient inequality  
    }   
    ...
    stop = (etaval + nu * zval) >= (rhs - tol); // check if new cut is satisfied by current solution
    if (not stop)                               // if not, then add cut
    {
      ++d_nCuts;                                // increment cut counter
      d_constrs.push_back(d_model.addConstr(d_eta + nu * d_z, GRB_GREATER_EQUAL, rhs));
