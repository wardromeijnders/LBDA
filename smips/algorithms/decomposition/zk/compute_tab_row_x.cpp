...
void ZK::compute_tab_row_x(double *tab_row_x, int nVarsMaster, int row, GRBmodel *master)
{
  ...
  for (size_t idx = 0; idx != nConsMaster; ++ idx)
  {
    ...
    {
      size_t pos = Brow.ind[nz];                // position of nonzero element
      vector<double> &T_row = d_Tmat[pos];      // corresponding row of T
      if (basic_var < T_row.size())             // check if element exists, otherwise it equals zero 
  ...
  */
  ...
  for (size_t idx = 1; idx != nVarsMaster; ++idx) // computing tab_row_x[idx]
  {
    ...
    {
      size_t pos = Brow.ind[nz];
      vector<double> &Trow = d_Tmat[pos];
      if (col < Trow.size())      // check if Trow[col] exists, otherwise it equals zero
