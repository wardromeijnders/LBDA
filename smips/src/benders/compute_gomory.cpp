    ...
    size_t s, int *vBasis, int *cBasis, double *ws, double *alpha)
{
    ...
    if (it == end)  // not visited, update and solve d_gomory
    {
        ...
        d_gomory.update(omega_alpha,
                        vBasis,
                        cBasis);  // update gomory relaxation

        gom_obj
            = d_gomory
                  .solve();  // solve gomory relaxation and store objective bound

        visited_bases.push_back(basis);  // basis is now visited: store it
        d_objectives[s].push_back(
            gom_obj);  // store corresponding objective value
    }
    else  // visited before
    {
        size_t idx = distance(begin, it);  // find index
        gom_obj
            = d_objectives[s][idx];  // and retrieve corresponding objective value
    }
