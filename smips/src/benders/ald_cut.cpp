...
void Benders::ald_cut(
    double *x, double *beta, double &tau, double &gamma, size_t maxRounds)
{
    std::vector<std::vector<double>> &omega = d_problem.d_omega;
    std::vector<double> &probs = d_problem.d_probs;
    ...
    tau = 0;
    gamma = 0;
    std::fill_n(beta, d_n1, 0.0);

    for (size_t s = 0; s != d_S; ++s)
    {
        double prob = probs[s];
        std::vector<double> &ws = omega[s];
