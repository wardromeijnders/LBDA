/*
    ...
    Lightly modified from the original source.
*/
...
// --- SET ARGUMENT ---
void Sarge::setArgument(std::string arg_short,
                        std::string arg_long,
                        std::string desc,
                        bool hasVal)
{
    ...
    {
        argNames.insert(
            std::pair<std::string, Argument *>(arg_short, args.back().get()));
    }

    if (!arg_long.empty())
    {
        argNames.insert(
            std::pair<std::string, Argument *>(arg_long, args.back().get()));
    }
}
...
{
    // The first argument is the name of the executable. After it we loop
    // through the remaining arguments, linking flags and values.
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        {
            if (!textArguments.empty())
            {
                std::cerr << "Flags not allowed after text arguments."
                          << std::endl;
            }

            // Parse flag.
            // First check for the long form.
            if (entry.compare(0, 2, "--") == 0)  // long form
            {
                // Erase the double dash since we no longer need it.
                entry.erase(0, 2);
                ...
                if (flag_it == argNames.end())
                {
                    ...
                    std::cerr << "Long flag " << entry << " wasn't found."
                              << std::endl;
                    return false;
                }
                ...
                {
                    // Next argument has to be a value string.
                    expectValue = true;
                }
            }
            else  // short form
            {
                ...
                // the last one is allowed to have an additional value following it.
                ...
                for (size_t i = 0; i < entry.length(); ++i)
                {
                    ...
                    {
                        // Flag wasn't found. Abort.
                        std::cerr << "Short flag " << k << " wasn't found."
                                  << std::endl;
                        return false;
                    }
                    ...
                    if (flag_it->second->hasValue)
                    {
                        ...
                        {
                            expectValue
                                = true;  // Next argument has to be a value string.
                        }
                    }
                }
            }
...
bool Sarge::getFlag(std::string arg_flag, std::string &arg_value)
{
    ...
    std::map<std::string, Argument *>::const_iterator it = argNames.find(
        arg_flag);
    if (it == argNames.end())
    {
        return false;
...
void Sarge::printHelp()
{
    ...
    for (it = args.cbegin(); it != args.cend(); ++it)
    {
        std::cout << ((*it)->arg_short.empty() ? "    "
                                               : "-" + (*it)->arg_short + ", ")
                  << "--" << (*it)->arg_long;
        std::cout << std::string(count - (*it)->arg_long.size(), ' ')
