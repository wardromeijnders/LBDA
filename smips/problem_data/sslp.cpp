...
void Problem::sslp(size_t nServers, size_t nClients, size_t S)
{
    ...
    ifstream lpFile;
    string filename = "data/sslp/sslp" + to_string(nServers) + "_"
                      + to_string(nClients) + ".lp";
    ...
    while (lpFile >> line)
    {
        ...
        if (not constraints)  // then we read an element of c or q
        {
                                 ...
                                 // case based on number of underscores
            {
                size_t pos1 = line.find("_");
                size_t pos2 = line.find("_", pos1 + 1);
                if (pos2 == string::npos)  // only one underscore has been
                                           // found: element of c
                {
    ...
    ifstream stoFile;
    string sto_name = "data/sslp/sslp_" + to_string(nServers) + "_"
                      + to_string(nClients) + "_" + to_string(S) + ".sto";
