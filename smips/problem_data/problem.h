...
{
public:
    size_t d_n1, d_p1, d_q1, d_m1, d_n2, d_p2, d_q2, d_m2,
        d_S;  // size parameters
    size_t d_fs_leq, d_fs_geq, d_ss_leq,
        d_ss_geq;  // number of >= and <= constraints in the first and second stage
    vector<double> d_l1, d_u1, d_l2, d_u2;
    vector<double> d_c, d_b, d_q;
    vector<vector<double>> d_Amat, d_Tmat, d_Wmat,
        d_omega;  // rows of d_omega correspond to scenarios
    ...
    void set_bounds(vector<double> &l1,
                    vector<double> &u1,
                    vector<double> &l2,
                    vector<double> &u2);
    ...
    double evaluate(
        double *x);  // evaluates cx + Q(x) (does not check feasibility)


    GRBModel d_sub;  // subproblem, used for evaluation of cx + Q(x) (more
                     // precisely, of v(omega, x))
    GRBConstr
        *d_constrs;  // useful to quickly update rhs of d_sub (heap allocated)
